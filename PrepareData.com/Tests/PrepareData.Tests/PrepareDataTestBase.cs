...
namespace PrepareData.Tests
{
    public abstract class PrepareDataTestBase : AbpIntegratedTestBase<PrepareDataTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PrepareDataTestBase()
        {
        ...
        protected void UsingDbContext(Action<PrepareDataDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PrepareDataDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PrepareDataDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PrepareDataDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PrepareDataDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PrepareDataDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PrepareDataDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PrepareDataDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PrepareDataDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PrepareDataDbContext, Task<T>> func)
        {
            T result;
